...
class VoltBroSensors {
	public:
		...
		static int32_t I2C_getRegister(uint8_t dev_addr, uint8_t register_addr, uint8_t num, bool unsign=true);//	Чтение данных от сенсора, NumByte - кол-во байт, Reg - адрес регистра, Uns - читать с учетом знака
		//
};
